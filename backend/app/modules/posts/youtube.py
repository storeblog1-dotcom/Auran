...
def _has_explicit_age_restriction(content_details: dict) -> bool:
    ...
    # Some videos include regional ratings. Reject explicit adult/restricted
    # values; an absent regional rating is not a restriction signal.
...
async def verify_youtube_watch_url(raw_url: str) -> VerifiedYouTubeVideo:
    ...
    if response.status_code != 200:
        raise BadRequestException("YouTube 영상 정보를 확인할 수 없습니다. 링크는 등록되지 않았습니다.")

    items = response.json().get("items") or []
    if len(items) != 1:
        raise BadRequestException("공개된 YouTube 일반 영상을 찾을 수 없습니다.")
